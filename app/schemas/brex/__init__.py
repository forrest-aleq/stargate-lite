...
BREX_CARD_UNLOCK = CapabilitySchema(
    ...
    description_detailed=(
        "Unlock a previously locked card to resume transactions. "
        "Card returns to active status."
    ),
    parameters={
        "access_token": ParameterSchema(
            type="string",
...
BREX_CARD_LIMIT_UPDATE = CapabilitySchema(
    ...
    description_detailed=(
        "Update the spending limit for an existing card. "
        "Can increase or decrease the limit."
    ),
    parameters={
        "access_token": ParameterSchema(
            type="string",
